package a08_반복;
/**
 * 프로그램은 내가 원하는 시점에 프로그램을 종료 할 수있어야하고 그전에는 계속 실행되어야한다.
 */
		...
		while(loopFlag) {
			...
			if(cmd == 0) {
				...
				while(true) {
					...
					if(cmd == 0) {
						System.out.println("프로그램 종료중...");
						loopFlag = false; //나가서까지 종료할 수 있도록 만드는 것
						break;
					}else if(cmd == 9) {
				...
				System.out.println("오류: 사용할 수 없는 명령어 입니다.");
			}
			System.out.println();
		}
